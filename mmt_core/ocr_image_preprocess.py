"""OCR provider image preprocessing restored from the old PaddleOCR-VL path."""
...
def preprocess_ocr_provider_image(image: Any) -> Image.Image:
    """Normalize and preprocess an OCR crop using the old PaddleOCR-VL logic."""

    pil_image = _normalize_image(image)
    return _preprocess_ocr_image(pil_image)


def save_ocr_provider_image(image: Any, output_path: Path | str) -> Path:
    """Preprocess one OCR crop and save it as a PNG for OCR providers."""
...
def _normalize_image(image: Any) -> Image.Image:
    ...
    if np is not None and isinstance(image, np.ndarray):
        ...
        raise RuntimeError(
            f"OCR provider preprocessing received unsupported numpy crop shape: {array.shape}"
        )

    raise RuntimeError("OCR provider preprocessing expects a PIL.Image or numpy array crop.")


def _preprocess_ocr_image(pil_image: Image.Image) -> Image.Image:
